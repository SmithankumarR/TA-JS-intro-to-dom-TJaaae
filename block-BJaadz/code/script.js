...
allHrs.forEach(elem => elem.style.backgroundColor = "antiquewhite");
// Change the 'border-radius' of all the hr to "5px" using array.
allHrsArray.style.borderRadius = `5px`;
// Change the alignment of the heading(h1) to center.
heading.style.textAlign = "center";
// Change the font size of the heading to 3rem.
heading.style.fontSize = "3rem";
// Change the border of hr with class 'image' to `2px solid purple`.
document.querySelector('image>hr').style.border = `2px solid purple`
// Hide the box number 17 (last box).
document.querySelector('seventeen').style.visibality = "hidden";
// Change the border of all the hr element from solid to dashed type
allHr.style.borderStyle = "dashed";
...
let rm = document.querySelector('one').children;
rm.remove();
...
let alternateBox = Array.from(allBoxes);
alternateBox.map(elem => (elem % 2 !== 0)).style.backgroundColor = `aliceblue`;
...
box2.classList.add('awesome-box');
...
let newImg = document.createElement('img').src = "https://images.unsplash.com/photo-1592500595497-d1f52a40b207?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=400&q=80";
...
input.children[0]= [placeholder = "Enter your email"];
...
anchor1.getAttribute('href').valueOf(`https://altcampus.school`)
anchor2.getAttribute('href').valueOf(`https://google.com`)
